...
import { BadgeDollarSign, CalendarDays, CircleCheck, Plus, Search, ShoppingBag, Pencil, Trash2 } from "lucide-react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Sheet, SheetContent, SheetDescription, SheetFooter, SheetHeader, SheetTitle } from "@/components/ui/sheet";
import { criarVenda, atualizarVenda, excluirVenda, listarVendas, listarClientesVenda, listarWebsitesVenda, vendasQueryKey, type Venda, type VendaInput, type VendaStatus } from "@/lib/vendas.query";
...
export function VendasPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async () => { await qc.invalidateQueries({ queryKey: vendasQueryKey }); toast.success("Venda excluída."); },
    onError: (e) => toast.error(e instanceof Error ? e.message : "Não foi possível excluir a venda."),
  });
  const handleDelete = (v: Venda) => {
    const confirmed = window.confirm(`Tem certeza que deseja excluir a venda "${v.titulo}"? Essa ação não pode ser desfeita.`);
    if (confirmed) deleteMutation.mutate(v.id);
  };
