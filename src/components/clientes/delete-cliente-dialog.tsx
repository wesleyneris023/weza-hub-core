    ...
    <AlertDialog open={Boolean(cliente)} onOpenChange={onOpenChange}>
      <AlertDialogContent className="border-glass-border bg-background">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            A exclusão de <strong>{cliente?.nome}</strong> é permanente e pode ser impedida caso
            existam registros relacionados.
          </AlertDialogDescription>
        </AlertDialogHeader>
        {hasLinks ? (
          <Alert variant="destructive">
            <AlertTriangle className="size-4" />
            <AlertTitle>Cliente com registros vinculados</AlertTitle>
            <AlertDescription>
              Trate os vínculos de websites, assinaturas, pagamentos, manutenções ou faturamentos
              antes da exclusão. O histórico financeiro não será apagado.
            </AlertDescription>
          </Alert>
        ) : null}
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            ...
            disabled={checking || deleting || hasLinks}
            className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
          >
            {checking ? "Verificando..." : deleting ? "Excluindo..." : "Excluir cliente"}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
